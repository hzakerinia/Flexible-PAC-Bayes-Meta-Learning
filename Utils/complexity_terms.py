...
def get_hyper_divergnce(prm, prior_model):   # corrected
    ...
    if not hasattr(prm, 'divergence_type') or prm.divergence_type == 'KL':
        ...
        hyper_dvrg = (norm_sqr + total_params * kappa_post**2) / (2 * kappa_prior**2) + total_params * (math.log(kappa_prior / kappa_post) - 1 / 2) # The baseline formula for KL-divergence was wrong and this is the correct form
...
def get_meta_complexity_term(hyper_kl, prm, n_train_tasks):   # corrected
    if n_train_tasks == 0:
        ...
        if prm.complexity_type in {'McAllester', 'Seeger'}:
            ...
            meta_complex_term = torch.zeros(1, requires_grad=False, device=prm.device)

        elif prm.complexity_type == 'LLA': # Our proposed method
            delta = prm.delta
            meta_complex_term = torch.sqrt(
                (hyper_kl + math.log(2 * math.sqrt(n_train_tasks) / delta)) / (2 * n_train_tasks))

        elif prm.complexity_type == 'Rezazadeh':
            delta = prm.delta
            meta_complex_term = (hyper_kl + math.log(2 * n_train_tasks / delta))
...
def get_task_complexity(prm, prior_model, post_model, n_samples, avg_empiric_loss, hyper_dvrg=0, n_train_tasks=1, dvrg=None, noised_prior=False):   # corrected
    ...
    if complexity_type == 'NoComplexity':
        ...
        complex_term = math.sqrt(1 / n_samples) * dvrg + hyper_dvrg * (
    ...
    elif complexity_type == 'LLA': # Our proposed method
        complex_term = (dvrg + (hyper_dvrg + math.log(8 * n_train_tasks * n_samples / delta) + 1) / n_train_tasks) /  (2 * n_samples)

    elif complexity_type == 'Rezazadeh':
        complex_term = (hyper_dvrg + dvrg + math.log(2 * n_samples * n_train_tasks / delta)) 
...
def  get_dvrg_element(post, prior, prm, noised_prior=False):
    ...
    if not hasattr(prm, 'divergence_type') or prm.divergence_type == 'KL':
        ...
        div_elem = 0.5 * torch.sum(prior_log_var - post['log_var'] + numerator / denominator - 1.0)
    ...
    # assert div_elem >= 0, f"number must be non-negative, got: {div_elem}"
    return div_elem
# -------------------------------------------------------------------------------------------
