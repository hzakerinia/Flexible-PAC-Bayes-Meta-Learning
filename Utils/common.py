...
def net_weights_diff(model1, model2, prm, p=2):   # corrected
    total_mag = torch.zeros(1, device=prm.device, requires_grad=True)[0]
    for (p1, param1), (p2, param2) in zip(model1.named_parameters(), model2.named_parameters()):
        total_mag = total_mag + (param1 - param2).pow(p).sum()
    return total_mag
