...
def run_meta_learning(data_loaders, prm):
    ...
    # -------------------------------------------------------------------------------------------
    def run_train_epoch(i_epoch, i_step = 0, prior_model_a=None):
        ...
        for i_meta_batch in range(n_meta_batches):
            ...
            total_objective, info = get_objective(prior_model, prm, mb_data_loaders,
                                                  mb_iterators, mb_posteriors_models, loss_criterion, n_train_tasks, prior_model_a)
    ...
    prior_model_a = pickle.loads(pickle.dumps(prior_model))
    
    # This is our special work which differs between the model for prior and initiation
    if prm.complexity_type == 'LLA': # Our proposed method
        for i in range(n_train_tasks):
            posteriors_models[i].load_state_dict(prior_model_a.state_dict())
        for i_epoch in range(prm.n_meta_test_epochs):
            i_step = run_train_epoch(i_epoch, i_step, prior_model_a)
    ...
    return prior_model, prior_model_a
