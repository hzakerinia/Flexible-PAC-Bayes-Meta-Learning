...
#
# -------------------------------------------------------------------------------------------
def get_objective(prior_model, prm, mb_data_loaders, mb_iterators, mb_posteriors_models, loss_criterion, n_train_tasks, prior_model_a=None):
    ...
    if prm.complexity_type == 'LLA': # Our proposed method
        if prior_model_a is None:
            hyper_dvrg *= 2
        else:
            hyper_dvrg += get_hyper_divergnce(prm, prior_model_a)
    ...
    if prm.complexity_type == 'Variational_Bayes':
        ...
        total_objective = avg_empiric_loss_per_task.mean() + torch.sqrt(complexity_per_task.mean()) + meta_complex_term

    elif prm.complexity_type == 'Rezazadeh':
        cf_env=2*(n_train_tasks-1)
        cf_tsk=2*(n_samples-1)
        tr=(cf_env+cf_tsk)/(cf_env*cf_tsk)
        total_objective =\
            avg_empiric_loss_per_task.mean() + torch.sqrt(tr*(complexity_per_task.mean() + meta_complex_term))
