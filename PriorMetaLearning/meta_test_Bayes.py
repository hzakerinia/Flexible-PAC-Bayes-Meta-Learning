...
def run_learning(task_data, prior_model_p, prior_model_a, prm, init_from_prior=True, verbose=1):
    ...
    if init_from_prior:
        post_model.load_state_dict(prior_model_a.state_dict())
    ...
    def run_train_epoch(i_epoch):
        ...
        for batch_idx, batch_data in enumerate(train_loader):
            ...
            complexity_term = get_task_complexity(prm, prior_model_p, post_model,  n_train_samples, avg_empiric_loss)
            ...
            if prm.complexity_type == 'Variational_Bayes':
                ...
                total_objective = avg_empiric_loss + torch.sqrt(complexity_term)
            elif prm.complexity_type == 'Rezazadeh':
                total_objective = avg_empiric_loss + torch.sqrt((complexity_term)/n_train_samples)
            else:
